...
function RateGraph({
  ...
  const checkArrowVisibility = () => {
    ...
    } else if (area.scrollLeft + area.offsetWidth + 2 >= area.scrollWidth) {
      setArrowVisibility({
        left: true,
        right: false,
      });
  ...
  return (
    <div className={styles["rate-graph-con"]}>
      ...
      {showArrows && (
        ...
        >
          <button
            className={cn(styles["arrow__main-con"], {
              [styles["disabled"]]: !arrowVisibility.left,
            })}
          ...
          <button
            className={cn(styles["arrow__main-con"], {
              [styles["disabled"]]: !arrowVisibility.right,
            })}
